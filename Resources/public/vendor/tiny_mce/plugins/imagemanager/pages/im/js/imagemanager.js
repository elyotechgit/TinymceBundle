(function($) {
	window.ImageManager = $.extend(BaseManager, {
		...
		init : function() {
			...
			t.return_sf_media_id = args.sf_media_id;
			if(args.multiSelect!="undefined" && args.multiSelect==true)
			{
				$("#multiselect_enable").removeAttr("disabled");
			}
			t.extensions = args.extensions;
			t.max_picture_allow = args.max_picture_allow;
			t.type_image = args.type_image;
			...
			$("#selectAllLink a").bind('click', function() {t.insertMultipleFiles();});
			...
			$('#filelist').click(function(e) {
				...
				if (a && a.rel) {
					...
					if (a.rel == 'file') {
						...
						{
							if($("#multiselect_enable").is(":checked"))
							{
								if($("#selectedPath").val()!="")
								{
									var count = $("#selectedPath").val().match(/,/g);
									if(count.length==t.max_picture_allow)
									{
										alert($.translate("{#error.max}").replace("{number}",t.max_picture_allow));
										return false
									}
								}
								// If multiselect is enable so I don't want to insertFiles, just select it
								$(e.target).parents(".thumbnail").find(".select input").trigger("click");
								var f_index = $(e.target).parents(".thumbnail").find(".select input").val();
								var path = t.files[f_index].path;
								if($(e.target).parents(".thumbnail").find(".select input").is(":checked")) $("#selectedPath").val($("#selectedPath").val() + ","+path);
								else $("#selectedPath").val(removePath($("#selectedPath").val(), path));
								var pathList = $("#selectedPath").val();
								if(pathList=="") $("#selectAllLink  .nb").html("(" + 0+")");
								else
								{
									var count = pathList.match(/,/g);
									$("#selectAllLink  .nb").html("(" + count.length  +")");
								}
								$("#selectAllLink").show();
							}else t.insertFiles(); 
			...
			$("#multiselect_enable").bind("change", function ()
			{
				if($(this).is(":checked")) $(".thumbnail .select").show();
				else $(".thumbnail .select").hide();
			});
			$("#multiselect_enable").trigger("change");
		...
		updateFileList : function() {
			...
			$(t.files).each(function(i) {
				...
				//All time i refresh, i need to check which files are selected.
				...
				if($("#multiselect_enable").is(":checked") && $("#selectedPath").val().indexOf(r.path)!=-1)
				{
					$("#file_" + i + " .select input").attr("checked", true);
				}
		...
		insertMultipleFiles: function ()
		{
			var t = this, s = t.currentWin.getArgs(), selectedPaths = [];
			
			var listFiles = $("#selectedPath").val().substr(1,$("#selectedPath").val().length).split(",");
			$(listFiles).each(function (i, checkbox)
			{	
				value = this+ "";
				...
				RPC.insertFiles({
					...
					document_base_url : s.document_base_url || s.default_base_url,
					no_host : s.remove_script_host || s.no_host,
					paths : selectedPaths,
					progress_message : $.translate("{#common.image_data}"),
					insert_filter : s.insert_filter,
					sf_media_id: t.return_sf_media_id,
					sf_media_type: t.type_image,
					oninsert : function(o) {
						t.currentWin.close();
						if (s.oninsert) {
...
{
	var pos = string.indexOf(path);
	var length = string.length;
	if(pos=="0") newstring = string.substr(path.length, length - pos );
	else
	{
	  newstring = string.substr(0,pos);
	  newstring += string.substr(pos+path.length,length); 
	}
	newstring = newstring.replace(",,",",");
	if(newstring.substr(newstring.legnth-2,newstring.length-1)) newstring = newstring.substr(0,newstring.length-1);
