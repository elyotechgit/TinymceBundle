/**
 * $Id: rpc.js 558 2008-11-05 15:54:42Z spocke $
 *
 * @author Moxiecode
			...
			if (err) {
				...
				$(this.toArray(res)).each(function(i, r) {
					...
					if (r.status && r.status.toLowerCase() != 'ok') {
						...
						root = args.visual_path.replace(/^\/([^\/]+)\/.*$/, '$1');
				...
				if (s.oninsert) {
					if (s.insert_filter) {
						$(o.files).each(function(i, f) {
							f.sf_media_id = s.sf_media_id;
							f.sf_media_type = s.sf_media_type
							s.insert_filter(f);
						});
					}
